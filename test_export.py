    ...
    with app.app_context():
        ...
        if response.ok:
            ...
            if task_id:
                ...
                while time.time() - start_time < 30:
                    ...
                    if result.ready():
                        # If task is ready, wait a bit for email processing
                        time.sleep(2)
                        break
                    time.sleep(1)
        ...
        if mailhog_response.ok:
            ...
            for msg in messages.get('items', []):
                ...
                # Check if this is an export email
                subject = msg.get('Content', {}).get('Headers', {}).get('Subject', [''])[0]
                if 'User Data Export' in subject:
                    print("Found export email!")
                    # Check for CSV attachment
                    has_attachment = 'Content-Type: text/csv' in msg.get('Raw', {}).get('Data', '')
                    print(f"Has CSV attachment: {has_attachment}")
        else:
