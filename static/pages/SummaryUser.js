...
<<<<<<< Updated upstream
    <div>
      <h1>Summary User</h1>
      {/* Summary and analytics interface for regular users */}
    </div>
  `,
=======
    <div class="container mt-4">
      ...
      <!-- Loading State -->
      <div v-if="isLoading" class="text-center my-5" role="status" aria-live="polite">
      ...
      <div v-if="error" class="alert alert-danger alert-dismissible fade show" role="alert">
        {{ error }}
        <button class="btn btn-link" @click="retryLoad">Retry</button>
        <button type="button" class="btn-close" @click="error = null" aria-label="Close"></button>
      </div>

      <!-- Auto-refresh Notice -->
      <div v-if="!isLoading && !error" class="text-center mb-3">
      ...
      <div class="row justify-content-center g-4" v-if="!isLoading && !error">
        ...
        <div class="col-12 col-md-6">
          <div class="card h-100">
            <div class="card-body">
              <h2 class="card-title text-center mb-3">Subject Performance</h2>
              <div class="text-center">
                <img 
                  v-if="charts.questions_chart"
                  :src="charts.questions_chart" 
                  alt="Subject-wise questions performance"
                  class="img-fluid" 
        ...
        <div class="col-12 col-md-6">
          <div class="card h-100">
            <div class="card-body">
              <h2 class="card-title text-center mb-3">Quiz Activity</h2>
              <div class="text-center">
                <img 
                  v-if="charts.attempts_chart"
                  :src="charts.attempts_chart" 
                  alt="Quiz attempts over time"
                  class="img-fluid" 
                  style="cursor: pointer"
      ...
      <div class="card mt-4">
        <div class="card-body text-center">
          ...
          <p class="text-muted">Export your complete quiz history for detailed analysis</p>
          <button 
          ...
          >
            ...
            {{ exporting ? 'Preparing Export...' : 'Export Quiz History' }}
          </button>
          <div 
            v-if="exportStatus" 
            :class="['alert mt-3', exportStatus.type]"
            role="alert"
          >
  ...
  methods: {
    async loadCharts() {
      ...
      try {
        const data = await api.get('/api/charts/user');
        this.charts = data;
        this.error = null;
    ...
    async handleExport() {
      ...
      try {
        const data = await api.post('/api/export/quiz-history');
        this.exportStatus = {
          type: 'alert-success',
    ...
    openModal(type) {
      if (type === 'questions') {
        this.modalImage = this.charts.questions_chart;
        this.modalTitle = 'Subject Performance Details';
      } else {
        this.modalImage = this.charts.attempts_chart;
        this.modalTitle = 'Quiz Activity Details';
      }
      this.resetView();
      this.modal.show();
    },
